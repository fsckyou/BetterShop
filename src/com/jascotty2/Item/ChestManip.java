...
public class ChestManip {
    ...
    public static int canHold(ItemStack[] items, Item check, boolean useMaxStack) {
        int amt = 0;
        if (check.isEntity()) {
            return -1;
        } else if (check.isKit()) {
            ...
            for (ItemStack item : items) {
                if (item == null || item.getAmount() == 0
                        || (check.equals(item) && item.getAmount() <= (useMaxStack ? check.maxStack : 64))) {
                    amt += (useMaxStack ? check.maxStack : 64) - (item == null ? 0 : item.getAmount());
                }
            }
        }
        return amt;
    ...
    public static int itemAmount(ItemStack[] items, Item check) {
        int amt = 0;
        for (ItemStack item : items) {
