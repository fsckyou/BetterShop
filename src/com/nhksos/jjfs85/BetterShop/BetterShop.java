...
import com.jascotty2.bukkit.FTPErrorReporter;
...
public class BetterShop extends JavaPlugin {
		...
		public void onPluginEnable(PluginEnableEvent event) {
			if (event.getPlugin().isEnabled()) { // double-checking enabled
				if (event.getPlugin().getDescription().getName().equals(
						"Help")) {
					shop.registerHelp();
				} else if (event.getPlugin().getDescription().getName().equals(
						"MinecraftIM")) {
	...
	private void hookDepends() {
		...
		test = getServer().getPluginManager().getPlugin("Spout");
		if (test != null) {
			Log("Spout Found! :)");
//			File spFile = new File(test.getClass().getProtectionDomain().getCodeSource().getLocation().getPath().
//				replace("%20", " ").replace("%25", "%"));
			try {
				super.getClassLoader().loadClass(test.getDescription().getMain());
				keyListener = new SpoutKeyListener();
				buttonListener = new SpoutPopupListener();
			} catch (ClassNotFoundException ex) {
				Log(Level.SEVERE, "Error loading Spout!", ex);
			}
		}
	}
	...
	public void onEnable() {
		...
		logger.log(Level.INFO, String.format("Loading %s version %s ...",
		...
		if (!pricelist.load()) {
			Log(Level.SEVERE, "cannot load pricelist: "
					+ pricelist.pricelistName(), false);
		...
		pm.registerEvent(Event.Type.ENTITY_TARGET, entityListener,
				Event.Priority.Normal, this);
		// for sign events
		pm.registerEvent(Event.Type.PLAYER_INTERACT, signShop,
				Event.Priority.Normal, this);
		pm.registerEvent(Event.Type.BLOCK_BREAK, signShop.signDestroy,
				Event.Priority.Normal, this);
		// pm.registerEvent(Event.Type.BLOCK_DAMAGE, signShop.signDestroy,
		// Priority.Normal, this);
		// pm.registerEvent(Event.Type.BLOCK_CANBUILD, signShop.buildStopper,
		// Priority.Normal, this);
		pm.registerEvent(Event.Type.BLOCK_PLACE, signShop.signDestroy,
				Event.Priority.Normal, this);
	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		String commandName = command.getName().toLowerCase(), argStr = Str.argStr(args);

		if (config.logCommands) {
			config.logCommand(sender instanceof Player ? ((Player) sender).getName() : "(console)", commandLabel + " " + argStr);
		}
		...
		try {
			...
			if (!BSEcon.active()
				...
				BSutils.sendMessage(sender,
						"\u00A74 BetterShop is missing a dependency. Check the console.");
			...
			if (commandName.equals("shop")) {
				if (args.length > 0) {
					if (args[0].equalsIgnoreCase("list")) {
						...
						if (BSutils.hasPermission(sender,
								BSutils.BetterShopPermission.ADMIN_RESTOCK,
								true)) {
							stock.Restock(true);
							sender.sendMessage("Stock set to initial values");
						}
					} else if (args[0].equalsIgnoreCase("backup")) {
						if (BSutils.hasPermission(
								sender,
								BSutils.BetterShopPermission.ADMIN_BACKUP,
								true)) {
							SimpleDateFormat formatter = new SimpleDateFormat(
									"_yyyy_MM_dd_HH-mm-ss");
							String backFname = BSConfig.pluginFolder.getPath()
									+ File.separatorChar + config.tableName
									+ formatter.format(new java.util.Date())
									+ ".csv";
							try {
								if (pricelist.saveFile(new File(backFname))) {
									sender.sendMessage("Backup saved as "
											+ backFname);
								}
							} catch (IOException ex) {
								Log(Level.SEVERE, "Failed to save backup file "
										+ backFname, ex);
								sender.sendMessage("\u00A74Failed to save backup file "
										+ backFname);
							}
						}
						return true;
						...
						if (sender.isOp()) {
							...
							if (Updater.downloadUpdate()) {
								Log("Update Downloaded: Restarting Server..");
								BSutils.sendMessage(sender,
										"Download Successful.. reloading server");
								// this.setEnabled(false);
							...
							BSutils.sendMessage(sender,
									"Only an OP can update the shop plugin");
						}
						return true;
					} else if (args[0].equalsIgnoreCase("ver")
							|| args[0].equalsIgnoreCase("version")) {
						// allow admin.info or developers access to plugin
						// status (so if i find a bug i can see if it's current)
						if (BSutils.hasPermission(sender,
								BSutils.BetterShopPermission.ADMIN_INFO, false)
								|| (sender instanceof Player && (((Player) sender).getDisplayName().equals("jascotty2") || ((Player) sender).getDisplayName().equals("jjfs85")))) {
							BSutils.sendMessage(sender, "version "
									+ pdfFile.getVersion());
			...
			if (commandName.equals("shopbuyagain")
					|| commandName.equals("shopsellagain")) {
				...
				if (commandName.equals("shopbuyagain")) {
					...
					if (action == null) {
						BSutils.sendMessage(sender,
								"You have no recent buying history");
						return true;
					} else {
			...
			if (!config.useGlobalCommandShop()
					&& Str.isIn(commandName, new String[]{"shopbuy",
						"shopbuyall", "shopbuystack", "shopsell",
						"shopsellall", "shopsellstack", /*
					 * "shoplist",
					 * "shopitems",
					 * "shopcheck",
					 * "shoplistkits",
			...
			if (commandName.equals("shoplist")) {
				...
				// ((Player) sender).getWorld().strikeLightning(((Player)
				// sender).getCompassTarget());
				// } else {
	...
	protected static void Log(Level loglevel, String txt, boolean sendReport) {
		logger.log(loglevel, String.format("[%s] %s", name, txt == null ? ""
				: txt));
		if (config != null) {
			if (sendReport && loglevel.intValue() > Level.WARNING.intValue()
			...
			if (messenger != null
				...
				messenger.sendNotify(String.format("[%s] %s", name,
						txt == null ? "" : txt));
			}
		}
	}
	...
	protected static void Log(Level loglevel, String txt, Exception params,
		...
		if (txt == null) {
			...
			logger.log(loglevel, String.format("[%s] %s", name,
					txt == null ? "" : txt), params);
			if (config != null) {
				if (sendReport
				...
				if (messenger != null
						&& loglevel.intValue() > Level.INFO.intValue()
						&& config.sendLogOnError) {
					messenger.sendNotify(String.format("[%s] %s%n%s", name,
							txt, params.getMessage(), Str.getStackStr(params)));
				}
			}
		}
	}
	...
	protected static void Log(Level loglevel, Exception err, boolean sendReport) {
		...
		if (config != null) {
			...
			if (messenger != null
					...
					&& config.sendLogOnError) {
				messenger.sendNotify(String.format("[%s] %s%n%s", name,
						err == null ? "? unknown exception ?" : err.getMessage(), Str.getStackStr(err)));
			}
		}
	}
	...
	static void sendErrorReport(String txt, Exception err) {
		...
		if (allow) {
			...
			try {
				errorReporter.SendReport(bettershopPlugin,
						/* obscureIP */ config != null ? !config.unMaskErrorID : true,
						/* customMessage */ config != null ? (config.customErrorMessage.length() > 0 ? config.customErrorMessage : null) : null,
						/* buildDate */ lastUpdatedStr,
						/* lastAction */ lastCommand,
	 ...
	 * Plugin pl;
	 * 
	 * public AdminCommandSender(Plugin here) { pl = here; }
	 * 
	 ...
	 * 
	 * public Server getServer() { return pl.getServer(); } }//
	 */
}
